...
class HqlParse(object):
    ...
    def clean_sql(self):
        self.sql = re.sub(r',\s*?\w+\(.*\)\s*?OVER\s*?\([A-Za-z0-9_ \n,]*?\)\s+(?:(as)?\s*?)(\w+)', r',\2', self.sql, flags=re.M | re.I)
        self.sql = re.sub(r'\s*?case\s+when.*?\s+as\s+(\w+)', r' \1', self.sql, flags=re.M | re.I | re.S)
        self.sql = re.sub(r'(\w+\s+?)as(\s+?\w+)', r'\2', self.sql, flags=re.M | re.S | re.I)
        self.sql = re.sub(r'decimal\((\d+),(\d+)\)', r'decimal#\1#\2#', self.sql, flags=re.M | re.S | re.I)  # 将decimal类型的括号转换
        return self.sql

    # 恢复 decimal类型
    def reduction_column(self, col):
        column = re.sub(r'decimal#(\d+)#(\d+)#', r'decimal(\1,\2)', col, flags=re.M | re.S | re.I)
        return column
    ...
    def __extract_from_token(self, statement):
        if statement.get_type() == "CREATE":
            ...
            self._tables.append({'table_name': self.extract_table_name(statement),
                                 ...
                                 'table_comment': self.extract_table_comment(statement.value)})
    ...
    def extract_definitions(self, token_list):
        ...
        for token in token_list.flatten():
            ...
            if token.match(sqlparse.tokens.Punctuation, ')'):  # 语句的结束
                ...
                if 'decimal' == token.value[:7]:
                    token.value = self.reduction_column(token.value)
                tmp.append(token)
        if tmp:
            definitions.append(tmp)
        return definitions
    ...
    def extract_table_comment(self, sql_str):
        comment = re.findall(r"\s*\)\s*?COMMENT\s+\'(.*?)'\s+", sql_str, re.M | re.I | re.S)
        if comment:
            return comment[0]
        else:
            return None

    def get_partition_col(self, sql_str):
        partition = re.findall(r"(?<!--)\s+?partitioned\s+by\s*?\((\w*?)\s+.*?\)\s*", sql_str, re.M | re.I | re.S)
        if partition:
            return partition[0]
        else:
            return None
    ...
    def get_pk_from_table(self, table_name):
        key_list = re.findall(r'.+FROM\s+' + table_name + '\s+?--\s*mini?_?size:\s*([\w, ]+?)\s+', self.sql, re.S | re.I | re.M)
        if key_list:
            return key_list[0]
        return None

    def get_comment_from_table(self, table_name):
        key_list = re.findall(r'.+FROM\s+' + table_name + '\s+?--\s*.*?\s+comment:\s*(.*?)\n', self.sql, re.S | re.I | re.M)
        if key_list:
            return key_list[0]
        return None
