...
def hive_2_excel(sql):
    ...
    # 读取表格数据
    new_workbook = load_workbook('static\数据API详细设计文档.xlsx')
    ...
    if insert_info['sub_tables']:
        ...
        targettable_comment = parse.get_comment_from_table(main_table['table_name'])
        for key in target_columns_dic.keys():
            if target_columns_dic[key] == main_alias_name and key in column_dic.keys():
                ...
                # 来源表描述
                new_sheet['I' + str(row_idx)].value = targettable_comment
                # 来源表字段名称
        ...
        def get_table_struct(table_info):
            ...
            return [table_columns, table_name, table_alias, table_res_etl, table_edge_list, table_info['table_name']]
        ...
        for sub_table in sub_table_list:
            ...
            full_table_name = sub_table[5]
            full_table_comment = parse.get_comment_from_table(full_table_name.strip())
            # 处理 sub_res
            sub_res_str = ''
            for res in sub_res:
                table_edge = re.findall(r'(\w+?)\.[\w =]+?(\w+?)\.', res, re.S | re.I | re.M)  # 表的连线
            ...
            for key in target_columns_dic.keys():
                if target_columns_dic[key] == sub_alias and key in column_dic.keys():
                    ...
                    # 来源表描述
                    new_sheet['I' + str(row_idx)].value = full_table_comment
                    # 来源表字段名称
                    new_sheet['J' + str(row_idx)].value = key
                    # 来源表字段类型
                    new_sheet['K' + str(row_idx)].value = column_dic[key]['type']
